...
type Backend interface {
	...
	GetVVM(ctx context.Context, msg core.Message, state *state.StateDB, header *types.Header, vmCfg vm.Config) (*vm.VVM, func() error, error)
	...
	return []rpc.API{
		{
			Namespace: "vap",
			Version:   "1.0",
			Service:   NewPublicVaporyAPI(apiBackend),
			Public:    true,
		}, {
			Namespace: "vap",
			Version:   "1.0",
