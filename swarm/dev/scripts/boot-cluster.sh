...
#
# The cluster contains a bootnode, a gvap node and multiple swarm nodes, with
...
# static gvap configuration
GVAP_IP="192.168.33.3"
GVAP_RPC_PORT="8545"
GVAP_RPC_URL="http://${GVAP_IP}:${GVAP_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gvap and
# swarm nodes
start_bootnode() {
...
# start_gvap_node starts a gvap node with --datadir pointing at <base-dir>/gvap
# and a single, unlocked account with password "gvap"
start_gvap_node() {
  local dir="${base_dir}/gvap"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gvap account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GVAP_IP}"
    --rpcport   "${GVAP_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gvap" "${GVAP_IP}" "$(which gvap)" ${args[@]}
}
...
create_account() {
  ...
  gvap --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
