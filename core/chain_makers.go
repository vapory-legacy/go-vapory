...
func GenerateChain(config *params.ChainConfig, parent *types.Block, engine consensus.Engine, db vapdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (vapdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := vapdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllVapashProtocolChanges, engine, vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, engine consensus.Engine, db vapdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, engine consensus.Engine, db vapdb.Database, seed int) []*types.Block {
