...
var (
	...
	configPrefix   = []byte("vapory-config-") // config prefix for the db
...
func WriteCanonicalHash(db vapdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db vapdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db vapdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db vapdb.Putter, hash common.Hash) error {
...
func WriteHeader(db vapdb.Putter, header *types.Header) error {
...
func WriteBody(db vapdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db vapdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db vapdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db vapdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db vapdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db vapdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db vapdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db vapdb.Database) vapdb.Database {
	return vapdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db vapdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db vapdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db vapdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
