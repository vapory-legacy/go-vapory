...
// Package vapstats implements the network stats reporting service.
package vapstats
...
import (
	...
	"github.com/vaporyco/go-vapory/common/mclock"
...
// Service implements an Vapory netstats reporting daemon that pushes local
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Vapstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the vapstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to vapstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to vapstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.vap != nil {
		...
		if block == nil {
			block = s.vap.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.vap.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.vap != nil {
			head = s.vap.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.vap != nil {
			block = s.vap.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to vapstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.vap != nil {
		pending, _ = s.vap.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to vapstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.vap != nil {
		mining = s.vap.Miner().Mining()
		hashrate = int(s.vap.Miner().HashRate())

		sync := s.vap.Downloader().Progress()
		syncing = s.vap.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.vap.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to vapstats")
