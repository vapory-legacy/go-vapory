...
func (n *Node) OpenDatabase(name string, cache, handles int) (vapdb.Database, error) {
	if n.config.DataDir == "" {
		return vapdb.NewMemDatabase()
	}
	return vapdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
