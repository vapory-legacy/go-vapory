...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gvap.LightScryptN, Gvap.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.vapory", new NodeConfig());
			node.start();
			...
			VaporyClient ec = node.getVaporyClient();
	...
	// signatures too. Regression test for go-vapory issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogvaper
	...
	workspace, err := ioutil.TempDir("", "gvap-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/vapory/gvaptest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gvap and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.vapory", "github.com/vaporyco/go-vapory/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gvap.aar"), "gvap.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "vapory", "gvaptest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
