...
// faucet is a Vapor faucet backed by a light client.
package main
...
var (
	...
	vapPortFlag = flag.Int("vapport", 10801, "Listener port for the devp2p connection")
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Vapors to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Vapory and the faucet")
)

var (
	vapor = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Vapors", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *vapPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Vapory address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Vapory light client.
type faucet struct {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Vapory light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := vap.DefaultConfig
...
// close terminates the Vapory connection and tears down the faucet.
...
// apiHandler handles requests for Vapor grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, vapor),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Vapory address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/vaporyco/go-vapory/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), vapor)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, vapor)
...
// the username, avatar URL and Vapory address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Vapory addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Vapory address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Vapory address and profile URL.
...
// returning the username, avatar URL and Vapory address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Vapory address,
