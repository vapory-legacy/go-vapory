...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{portFull: 10801, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	fmt.Printf("How many Vapors to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.vapstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.vapstats = w.readString() + ":" + w.conf.vapstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.vapstats)
		infos.node.vapstats = w.readDefaultString(infos.node.vapstats) + ":" + w.conf.vapstats
	}
