...
// makeWizard creates and returns a new vappupp wizard.
...
// setting up a new or managing an existing Vapory private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to vappupp, your Vapory private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Vapory network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and vapstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Vappupp uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Vapory network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".vappupp", w.network)
