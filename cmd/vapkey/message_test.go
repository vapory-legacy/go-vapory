...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "vapkey-test")
	if err != nil {
	...
	// Create the key.
	generate := runVapkey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runVapkey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runVapkey(t, "verifymessage", address, signature, message)
