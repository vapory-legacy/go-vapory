...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Vapory
...
func (vap *Vapory) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-vap.shutdownChan:
					return

				case request := <-vap.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(vap.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := core.GetBloomBits(vap.chainDb, task.Bit, section, head); err == nil {
...
// for the Vapory header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  vapdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db vapdb.Database, size uint64) *core.ChainIndexer {
	...
	table := vapdb.NewTable(db, string(core.BloomBitsIndexPrefix))
