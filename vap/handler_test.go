...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*vapdb.MemDatabase).Keys() {
	...
	statedb, _ := vapdb.NewMemDatabase()
...
// Tests that post vap protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, vvmux, new(testTxPool), pow, blockchain, db)
	if err != nil {
	...
	peer, _ := newTestPeer("peer", vap63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, vapash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
