...
import (
	...
	"github.com/vaporyco/go-vapory/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gvap console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = vap.getBlock(vap.getTransaction(tx).blockHash);
  var genesis = vap.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.vap.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  vap.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			// Create the tracer, the VVM environment and run it
			tracer, err := New("callTracer")
			if err != nil {
				t.Fatalf("failed to create call tracer: %v", err)
			}
			vvm := vm.NewVVM(context, statedb, test.Genesis.Config, vm.Config{Debug: true, Tracer: tracer})

			msg, err := tx.AsMessage(signer)
			if err != nil {
				t.Fatalf("failed to prepare transaction for tracing: %v", err)
			}
			st := core.NewStateTransition(vvm, msg, new(core.GasPool).AddGas(tx.Gas()))
