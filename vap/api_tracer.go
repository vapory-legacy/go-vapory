...
type ephemeralDatabase struct {
	diskdb vapdb.Database     // Persistent disk database to fall back to with reads
	memdb  *vapdb.MemDatabase // Ephemeral memory database for primary reads and writes
}
...
func (db *ephemeralDatabase) NewBatch() vapdb.Batch {
...
func (db *ephemeralDatabase) Prune(root common.Hash) {
	...
	db.memdb, _ = vapdb.NewMemDatabaseWithCap(db.memdb.Len())
	if _, err := sync.Commit(db); err != nil {
		panic(err) // writing into a memdb cannot fail
	}
}

// TraceChain returns the structured logs created during the execution of VVM
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.vap.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.vap.blockchain.CurrentBlock()
	default:
		from = api.vap.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.vap.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.vap.blockchain.CurrentBlock()
	default:
		to = api.vap.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	memdb, _ := vapdb.NewMemDatabase()
	db := &ephemeralDatabase{
		diskdb: api.vap.ChainDb(),
		memdb:  memdb,
	}
	if number := start.NumberU64(); number > 0 {
		start = api.vap.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewVVMContext(msg, task.block.Header(), api.vap.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.vap.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.vap.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
// VVM and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.vap.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.vap.blockchain.CurrentBlock()
	default:
		block = api.vap.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.vap.blockchain.GetBlockByHash(hash)
	if block == nil {
...
// TraceBlock returns the structured logs created during the execution of VVM
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.vap.engine.VerifyHeader(api.vap.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.vap.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewVVMContext(msg, block.Header(), api.vap.blockchain, nil)
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewVVM(vmctx, statedb, api.config, vm.Config{})
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.vap.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.vap.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
// TraceTransaction returns the structured logs created during the execution of VVM
// and returns them as a JSON object.
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	// Retrieve the transaction and assemble its VVM context
	tx, blockHash, _, index := core.GetTransaction(api.vap.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewVVM(vmctx, statedb, api.config, vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.vap.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewVVMContext(msg, block.Header(), api.vap.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewVVM(context, statedb, api.config, vm.Config{})
