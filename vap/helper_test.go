...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, error) {
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, vapash.NewFaker(), db, blocks, generator)
	if _, err := blockchain.InsertChain(chain); err != nil {
		panic(err)
	}

	pm, err := NewProtocolManager(gspec.Config, mode, DefaultConfig.NetworkId, vvmux, &testTxPool{added: newtx}, engine, blockchain, db)
	if err != nil {
		return nil, err
	}
	pm.Start(1000)
