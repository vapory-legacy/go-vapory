...
// Package bind generates Vapory contract Go bindings.
//
// Detailed usage document and tutorial available on the go-vapory Wiki page:
// https://github.com/vaporyco/go-vapory/wiki/Native-DApps:-Go-bindings-to-Vapory-contracts
package bind
...
func Bind(types []string, abis []string, bytecodes []string, pkg string, lang Lang) (string, error) {
	...
	for i := 0; i < len(types); i++ {
		...
		vvmABI, err := abi.JSON(strings.NewReader(abis[i]))
		if err != nil {
			return "", err
		}
		...
		for _, original := range vvmABI.Methods {
		...
		contracts[types[i]] = &tmplContract{
			...
			Constructor: vvmABI.Constructor,
			Calls:       calls,
			Transacts:   transacts,
		}
	}
