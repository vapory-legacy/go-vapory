...
import (
	...
	"github.com/vaporyco/go-vapory/accounts/usbwallet/internal/trezor"
...
// or whether the Vapory app was not started on it.
...
// Vapory address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.VaporyAddress)
	if _, err := w.trezorExchange(&trezor.VaporyGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.VaporyTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.VaporyTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
