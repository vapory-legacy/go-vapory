...
func TestClientSubscribeInvalidArg(t *testing.T) {
	...
	check := func(shouldPanic bool, arg interface{}) {
		...
		client.VapSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("vap", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.VapSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("vap", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.VapSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.VapSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "vap_echo", i)
			if err != nil {
				if !wantError {
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-vapory-test-ipc-%d-%d", os.Getpid(), rand.Int63())
