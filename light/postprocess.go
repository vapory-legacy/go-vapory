...
import (
	...
	"github.com/vaporyco/go-vapory/common/bitutil"
...
func GetChtRoot(db vapdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db vapdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db vapdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db vapdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := vapdb.NewTable(db, ChtTablePrefix)
	idb := vapdb.NewTable(db, "chtIndex-")
...
const (
	...
	vapBloomBitsSection       = 4096
	vapBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db vapdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db vapdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db vapdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := vapdb.NewTable(db, BloomTrieTablePrefix)
	idb := vapdb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = vapBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-vapBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
