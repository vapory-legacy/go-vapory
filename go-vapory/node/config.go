...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gvap is "gvap". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gvap", keep that.
	if name == "gvap" || name == "gvap-testnet" {
		name = "Gvap"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gvap" instances.
var isOldGvapResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gvap 1.4 are used if they exist.
	if c.name() == "gvap" && isOldGvapResource[path] {
		oldpath := ""
		if c.Name == "gvap" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-vapory-keystore")
		ephemeral = keydir
	}
