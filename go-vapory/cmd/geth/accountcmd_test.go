...
func TestAccountListEmpty(t *testing.T) {
	gvap := runGvap(t, "account", "list")
	gvap.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gvap := runGvap(t, "account", "list", "--datadir", datadir)
	defer gvap.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gvap := runGvap(t, "account", "new", "--lightkdf")
	defer gvap.ExpectExit()
	gvap.Expect(`
...
`)
	gvap.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gvap := runGvap(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gvap.ExpectExit()
	gvap.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gvap.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gvap.StderrText(), m) {
...
// https://github.com/vaporyco/go-vapory/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gvap.SetTemplateFunc("keypath", func(file string) string {
